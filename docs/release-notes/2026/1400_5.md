...
layout: home
---
# 2024.1400.5 - Current Version: IDE 1065/Runtime 1031

Download Links: [Windows](https://gms.yoyogames.com/GameMaker-Installer-2024.1400.5.1065.exe) - [macOS](https://gms.yoyogames.com/GameMaker-2024.1400.5.1065.pkg) - [Ubuntu Beta](https://gms.yoyogames.com/GameMaker-Beta-2024.1400.5.1065.deb)
...
## 2024.1400.5 Release 5 (IDE 1065 / GMS2 RT 1031) - [Changes](https://github.com/YoYoGames/GameMaker-Bugs/issues?q=is%3Aissue%20is%3Aclosed%20milestone%3A"LTS%202026.0"%20closed%3A2026-04-29..2026-05-12%20sort%3Aupdated-desc) (13/05/2026)

### Features Added Or Changed

- **Stability: Added An "Emergency Restart" System For When GPU Driver Errors Occur**
    - Over the timeframe of the .1400/.14 series of releases we have had a lot of reports for GPU driver fails for a variety of reasons
    - However, because the vast majority of users who get these issues are not logged-in and choose to send their reports anonymously, we're getting a lot of duplicate reports that we also could not reach out and get extra information about
    - Now, when GameMaker detects there has been a GPU error (and only a GPU error) the IDE will do the following things:
        - Try to save as much of the project as possible
        - Send us an analytic event detailing the reason for the restart (so we're still getting the information about the crash happening - just now in a very streamlined, automatic way)
        - Restarts the IDE
        - Reloads the project and your layout, so you end up back where you were when the crash happened
    - Note that this restart does not spawn the bug reporter when GameMaker starts itself back up (because the analytic was already sent)
    - The only caveat here is that if you were editing a sprite in the Image Editor at the time of the crash, then because the edits were in the GPU's memory, there is a strong chance we were not able to save any unsaved recent changes

- **Stability: Reduced Resource Usage When Editing Sprites**
    - And to go alongside that above change, the way that sprites are loaded into memory has now been changed in order to reduce the amount of RAM and VRAM that is required
    - This also avoids a format conversion and so CPU work is lower now as well
    - So this should reduce instances of GPU issues as you're opening/editing your assets

- **Package Manager: Removed "npmjs" config upstream integration [#14605](https://github.com/YoYoGames/GameMaker-Bugs/issues/14605)**
    - In the majority of cases GameMaker will not require remote scripts to be run on our nodes, and so this functionality has now been disabled by default
    - However, if you do need to add an NPM source inside Package Manager and that node makes use of remote scripts, then there is a checkbox to allow the functionality on _that_ node

<br>

### Crash/Stability Fixes

- Debugger: [Mac IDE] GM no longer crashes when accidentally double-clicking to start two simultaneous debug runs for macOS [#14761](https://github.com/YoYoGames/GameMaker-Bugs/issues/14761)
- Shutdown: GM no longer crashes when cleaning up processes while exiting (which previously gave a "System.Management.ManagementException: Invalid class" error in earlier releases) [#14735](https://github.com/YoYoGames/GameMaker-Bugs/issues/14735)

<br>

### IDE Bugs Fixed

- Game Options : [Android] Tweaked the `Build Settings > Architecture` label to clarify ARM64 includes Armv8 devices [#14840](https://github.com/YoYoGames/GameMaker-Bugs/issues/14840)
- Project Import: Importing a .yymps no longer duplicates its contents to GM's "ProgramData" folder also [#14794](https://github.com/YoYoGames/GameMaker-Bugs/issues/14794)
- Local Package Integration: Create dialog now allows removing folders which contain Included Files from your current selection [#14796](https://github.com/YoYoGames/GameMaker-Bugs/issues/14796)
- Local Package Integration: Included Files are no longer shown twice in your assets selection [#14782](https://github.com/YoYoGames/GameMaker-Bugs/issues/14782)
- Package Manager: Updated the bundled version of Node to stop permissions/connection issues on some user's machines that resulted in an endless spinner for all sources [#12908](https://github.com/YoYoGames/GameMaker-Bugs/issues/12908)

<br>

### In-Game Bugs Fixed

- In-Game: [Android] Audio no longer crashes in games when they are paused due to a permissions dialog or ad, if built with 2024.1400.0.4.1027 RTs or newer [#14806](https://github.com/YoYoGames/GameMaker-Bugs/issues/14806)
...
- Manual Content: Flex Panel docs have better info regarding editing `layerElements` at runtime (`elementId` usage) [#14188](https://github.com/YoYoGames/GameMaker-Bugs/issues/14188)
- Manual Content: Strings page is no longer absent from the Index (A-Z) listing [#14647](https://github.com/YoYoGames/GameMaker-Bugs/issues/14647)
- Manual Content: File Watcher page now documents the "Project Modified" dialog and its two butons "Reload" and "Save" [#14742](https://github.com/YoYoGames/GameMaker-Bugs/issues/14742)
- Manual Content: `ds_grid_read()` page now says that this function will resize the grid if necessary [#14414](https://github.com/YoYoGames/GameMaker-Bugs/issues/14414)
- Manual Content: `window_set_cursor()` page now documents that "cr_size_all" (and others) are not allowed on macOS, so a custom sprite must be used [#14664](https://github.com/YoYoGames/GameMaker-Bugs/issues/14664)

<hr>
<br>

## 2024.1400.5 Release 4 (IDE 1055 / GMS2 RT 1027) - [Changes](https://github.com/YoYoGames/GameMaker-Bugs/issues?q=is%3Aissue%20is%3Aclosed%20milestone%3A"LTS%202026.0"%20closed%3A2026-04-23..2026-04-28%20sort%3Aupdated-desc) (29/04/2026)

### Crash/Stability Fixes

- Startup: [Windows IDE] All plugins are now signed, which stops issues where Windows' "Smart App Control" feature in Windows Defender closes GM during the "Loading Plugins" phase of the splash screen [#14724](https://github.com/YoYoGames/GameMaker-Bugs/issues/14724)

<br>

### Build Bugs Fixed

- Building Projects: Fixed that macros inside a Prefab which reference another macro became variable references [#14701](https://github.com/YoYoGames/GameMaker-Bugs/issues/14701)

<br>

### In-Game Bugs Fixed

- In-Game: `image_index` is no longer always "NaN" for Objects that have no sprite set, as seen in 2024.1400.5.1024 RTs [#14727](https://github.com/YoYoGames/GameMaker-Bugs/issues/14727)
- UI Layers: Fixed that flexpanel auto-sizing with a stretched and maintain-aspect-ratio sprite element still used that sprite's original width and height [#14726](https://github.com/YoYoGames/GameMaker-Bugs/issues/14726)
- In-Game: [HTML5] `audio_sound_length()` no longer causes exceptions if called on non-streamed audio in unloaded groups, as it now knows the length of these files also [#14712](https://github.com/YoYoGames/GameMaker-Bugs/issues/14712)
- In-Game: [YYC] `static_get()` trying to access a method's statics no longer gives the exception seen in 2024.1400.5.1024 RTs [#14721](https://github.com/YoYoGames/GameMaker-Bugs/issues/14721)

<br>

### Docs Bugs Fixed

- Manual Content: GML Visual Reference page now uses the correct URL for the "Instance Actions" link [#14705](https://github.com/YoYoGames/GameMaker-Bugs/issues/14705)
...
- Building Projects: Fixed that Included Files were not being included in builds made with 2024.1400.5 RTs [#14698](https://github.com/YoYoGames/GameMaker-Bugs/issues/14698)
- Building Projects: [YYC] Projects no longer fail to compile due to "System.InvalidOperationException: Stack empty." errors [#14699](https://github.com/YoYoGames/GameMaker-Bugs/issues/14699)
...
- **Commandline Builds: New compile-only setting for Igor [#9367](https://github.com/YoYoGames/GameMaker-Bugs/issues/9367)**
