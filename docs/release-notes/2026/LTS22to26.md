### Headline Features Added Or Changed

- **New Sign In/Out Flow**
    - You are no longer always required to sign-in during startup
        - Instead, you can start GameMaker as a “guest” account and do most actions inside the IDE without needing to be signed-in
        - You will still need to sign-in before you can do something which definitely needs to check your licence, like clicking the Create Exe button to start a build or upload a game to the GX Community (if you're a guest user, then a prompt to sign-in now appears on-demand when clicking a platform inside Target Manager or the run/create exe, etc., buttons on the menubar)
        - Once you do sign-in, then this is of course maintained each time you start GM later on
        - Note that if you are a new user and you use GameMaker as a guest for a while and configure your Preferences and Device Manager list, etc., and then sign-in to an account, if no user settings folder already exists for your new account all of your guest settings will be copied over, so you won’t need to set things up again
        - You can sign out and return to using a guest account later if you wish - but do always remember this will cause things like Preferences, device lists, and the Start Page’s recent projects list to all change accordingly (likely reset to defaults) and your changed settings will not be copied over the next time you sign-in to your real account again
    ...
    - The menubar commands for "Logout" and “My Account” have been moved from "File" to a dedicated new "Account" menu on the menubar, along with some new commands for logging-in/out on-demand
        - The old command "My Account" has also been split into two commands so you can reach your GameMaker Account and Opera Account pages more easily
    - Signing-in no longer has different UI depending on whether you have an old "Legacy" account or a newer Opera SSO Account
        - The "Legacy" UI for signing-in has been removed from inside GameMaker and instead _all_ users will sign-in via the one button, which will open your web browser and allow you to sign-in there - once on the website, you will find the Legacy option alongside the SSO one
        - A reminder that none of these changes affect the usual 28-day licensing requirement for users who are signed-in to renew their login, nor does it remove any other internet access requirements
        - **Proxy server users**: Be aware this release also moves the controls for saying whether you have a proxy server and which details you log in with, so you will now find these in `Preferences > General Settings > Proxy Settings`. (Any proxy values have always applied to all internet traffic, including to SSO sign-ins, so this should also clear up any confusion now that the settings are in a more "global" location.)

- **GMS2 Runtimes: Changes To Names And How The Runtimes Download**
    - Everywhere that refers to your runtimes installation, you will now see "VM" has been renamed "GMS2 VM" and "YYC" is "GMS2 YYC"
       - This change is simply to emphasise that the new GMRT runtime is in development and to clarify that new features at that time may not go into the "old" runtimes also
    - You will see a new "Runtime Modules" dialog for customising what GMS2 runtimes you download the first time you start this new LTS version, so for more information on this dialog and how to use it, please see [our new FAQ all about it](https://help.yoyogames.com/hc/en-us/articles/7303279382173)
        - GameMaker now handles downloading just the specific modules you require from each GMS2 runtime - you can select a minimal install suitable for your current OS, whichever modules you require just now, or “just give me everything” (which is what GameMaker always did in older LTS releases)
        - Similarly, in all older GM versions, the "Base" module always included the tools required for all three of the Windows, Mac and Ubuntu IDEs, regardless of which one you were using - and so the download size had grown over time, but a reasonable amount of the package was never relevant for your specific installation. Now, the "base" entry in the Runtime Modules dialog within the IDE will only download the tools relevant for the one IDE/operating system you’re using, saving you time downloading the runtimes and lessening the space required on your disk
        - (The same change applies if you’re using Igor to download your runtimes via command line tools)
        - You can modify these choices at any time by clicking the new button on the `Runtime Feeds` section of Preferences, but be aware that whilst the dialog will install additional modules okay, it will not remove any which are currently downloaded  - if you want to do that, you will need to modify the dialog settings and then delete your entire runtime folder outside GameMaker, so that GameMaker will re-download just the bits you now want next time you start it up
    - Also, GameMaker no longer forces installing the original runtime for an IDE release if a suitable newer runtime is active [#5927](https://github.com/YoYoGames/GameMaker-Bugs/issues/5927) 
        - For example, this 2026.0 GameMaker IDE will get its matching 2026.0 GMS2 runtime and if we release a newer 2026.0 runtime you could install that one and set it active, then (if you wished) delete the original runtime manually to save disk space
        - Note that the newer runtime must be from the same "family" as the IDE version - having a "2026.1" runtime (for example) will still not be accepted by this 2026.0 IDE and it would download its matching runtime again for you
    - Target Manager also now features status icons to tell you which runtimes have been installed already
    - And there is a new Preference for controlling your preferred default target (by default it remains whichever of Windows/Mac/Ubuntu you're using to run GameMaker itself)
    - Preferences has been also changed so that downloading any GMS2 runtime version manually will always ask if you want to activate it, regardless of where on that line you clicked when initiating the download (previously, you had to specifically click the version number in order to be asked)
    - As well as all the above, there have been a number of fixes for RSS downloading crashes or when for some reason your GMS2 runtime download has not been set as active
...
- **Welcome Tab**
    ...
    - (Obviously, if you have it open when you close a project it will form part of _that_ project's layout and therefore will still be there the next time you open that one project, so bear that in mind)

- **New Project Loading/Saving/Importing/Exporting Tool**
    - GameMaker now has a new `ProjectTool` application it calls when managing projects, which performs a lot of upgrade/downgrade checks on the project as you're opening them
    ...
    - It will also allow LTS26 IDEs to open projects which were created/opened in recent or future IDE releases and for you to be able to edit them, as now IDEs will know to safely ignore the assets and other features they do  not know about
    - {% include important.html content="If you have any issues opening/importing projects or packages, please ensure you're using the newest release of ProjectTool inside `Package Manager` and then try again **before** you submit any bugs!" %}

- **Code Editor 2 Now Available For Use**
    ...
    - You can now add *.tmSnippet files into your user directory with the name as a built-in snippet to override that snippet [#10164](https://github.com/YoYoGames/GameMaker-Bugs/issues/10164)
        ...
        - Note that struct-based snippets will require you to escape the "$" character that's normally used as the line break character, and you do this by using a backslash before it ("\$") [#12827](https://github.com/YoYoGames/GameMaker-Bugs/issues/12827)

- **Feather: Enabled By Default And Extended Greatly**
    - Feather is now enabled by default if you're a Code Editor 1 user (Code Editor 2 uses the newer "Diagnostics" system, which does all the same things and more)
    - As a reminder, Feather will help catch coding issues sooner and point out improvements you can make (often automatically, via the Quick Rules integration) - there is a Preference for controlling whether you want more "relaxed" checking or "strict"
    - You can control in Preferences the amount of warnings Feather will show you in the code editor: "None" (which is compile errors only), "Syntax Errors" (which is syntax errors and compile errors),"Type Errors" (which adds variable type enforcement warnings), and "All" (everything Feather offers)
    - You can also supply optional naming convention rules in Preferences, and these will be used when Feather reviews your code to help make sure you're following your own coding standards throughout
    ...
    - Extended the "// feather" directives by adding an "in" suffix, which takes a case-insensitive file path string (matching the asset browser's folder structure for your project), plus the rule(s) you wish to be ignored
        ...
        - Note that if you have two rules which affect the same path/object, then there is no guarantee which will be applied - and if you have lots of custom rules in your scripts or rules which affect a lot of scripts, then this may cause a performance overhead
    ...
    - Also fixed a number of Syntax Errors issues - so now if you do turn Feather off, the "old" errors are more reliable and the dialog populates itself automatically whenever you open it

- **UI Layers And Flexpanels [#3159](https://github.com/YoYoGames/GameMaker-Bugs/issues/3159)**
    - You can now add special layers into your rooms to hold items in a flexible predefined layout, called a Flexpanel, which will scale content on the layer automatically to fit the device display or the dimensions of a viewport
    - In this way, menu screens, HUDs, and lots of other content can be laid out in the room editor and then updated in-game, if required
    - Layers are "global" and so you can create them in any room and they will appear in all others (both in the IDE and in-game), so that you can simply toggle their visibility on/off as you require, rather than have to recreate a layer multiple times across your individual rooms
    - You will find many controls for sizing, wrapping, anchor locations, margins, etc., all in the IDE - and your panels can be modified via the Inspector or via mouse controls
    - There are a large number of functions (far too many to list here) for creating layers from scratch in code or for manipulating content on existing IDE layers, so please do see the manual pages for "UI Layers" and "Flexpanels" for all the info on these
    - Be aware that `mouse_x` and `mouse_y` for instances on UI Layers use GUI co-ords rather than regular layer co-ords [#11678](https://github.com/YoYoGames/GameMaker-Bugs/issues/11678) and because they are in UI space they will be shown above any black bar areas surrounding your game’s `application_surface`

- **Filters/Effects: Added More Layer Types**
...
- **Particle Asset Type And A New Editor**
    ...
    - These are "Electricity", "Embers", "Embers 2", "Fire", "Flame Intensity", "Rain", "Smoke", "Smoke 2", "Sparks", "Warp Centre", and "Warp Lines"
    - The GML Visual "Create Particle System" node also has a picker for selecting an optional particle system asset
    - You can set a timer for repeatedly bursting particles inside the IDE, allowing you to better preview your effects (which gives the same end result as the also-new functions `part_emitter_delay()` and `part_emitter_interval()` which you can now use in your code, but allows you to set the initial values up in the editor)
    - Note: Be aware that older IDEs and runtimes have no knowledge of this new particles asset type, and so if you open or create any project in this new release it will not be editable in older GameMaker versions and instead will open in those older IDEs in read-only mode (significantly older IDEs will just show an error on loading the project)

- **Prefab Library [#4681](https://github.com/YoYoGames/GameMaker-Bugs/issues/4681)**
    - You can now enable the new Prefab Library and import our initial batch of Prefabs into your projects
    ...
    - Open the Library inside your project by clicking `Windows > Prefab Library` and this will open the new bottom dock panel, Prefabs are then downloaded (once) using its Package Manager integration and then added to any of your projects thereafter from your library
    - You're able to preview the code in a prefab's events and also view any Notes it contains without having to add these files to your project first (if a Note is a markdown file GM will use the markdown viewer for the preview of that file, rather than showing you the raw text in an editor)
    ...
    - You can duplicate the full contents of folders or entire prefab collections at once, rather than having to add each prefab individually across multiple operations
    - Note that vast majority of values relating to prefabs can be customised after linking, which means there is usually no need to duplicate a prefab into your project just so you can change some of its basic values
    ...
    - You will also see that all prefabs are shown in an orange colour to distinguish when their source comes from something external to your project
    ...
    - Any exposed macros found in a prefab you're using in your project can be modified also [#9781](https://github.com/YoYoGames/GameMaker-Bugs/issues/9781)
        - This uses a syntax similar to the config macros functionality that GM has supported for a long time - `#macro ::<prefab-scope>::[<ConfigName>:]<MacroName> <value>` - e.g., `#macro ::com.russell.macrotest-1.0.0::Release:IsDebug false` as shown in the comments on the issue above
    - Of course, when duplicating Included Files from a prefab into your project they'll go directly into the datafiles folder of the project as per normal and therefore do not appear in the Asset Browser tree
    - You can also reinstall a Prefab Collection via the right-click menu when selecting the collection - in this way, you can "undo" customisations made to that collection's prefabs that you no longer want and instead go back to a fresh start
...
- **Windows ARM64 Device Support**
    ...
    - For Run/Debug, there is no choice you need to make and your game will detect during start-up which architecture the target device uses and automatically run the correct runner (i.e. x64 or ARM64)
    - For Create Exe, there are new choices on the packaging type dialog so that, as well as the existing package types, you can now choose to build an ARM64 Zip or an ARM64 NSIS .exe
...
- **Switch 2 Target Added**
    - Switch 2 is now a target for GameMaker
    - This uses a very similar setup to Switch 1, but please do see the setup guides before attempting any builds or filing any bugs
    - See the Switch 2 repository's own wiki - which you can easily reach via [the wiki front page](https://github.com/YoYoGames/GameMaker-Bugs/wiki/)
...
- **Start Page Now Has More Project Types To Choose From**
    - In order for us to add more Templates for the Games and Live Wallpapers project types that LTS26 brings support for (see below), the Start Page has now been tweaked to section-off each of these
    - This way, we can add more projects for you to use in future without making the old combined list unnecessarily large
    - Your custom templates will still appear as normal

- **[GX.games] Export your very own Game Strip or Live Wallpaper**
    - You're now able to create a Game Strip, which is a banner-sized game (for example: an advert, a platformer game, or even a virtual pet app) that runs when doing regular web-browsing or your home/start page inside the Opera GX browser 
    - And you can create a Live Wallpaper, which is a project which runs as the wallpaper on your Windows or Mac desktop and can be interacted with by you or by the system itself in order to animate things or change images, etc.
    - Both of these are uploaded to the GX Community - and Live Wallpapers require you to run a small companion app in order to load and use the wallpaper on your local machine

- **Package Manager**
    - Package Manager is how GameMaker now delivers prefabs (which includes all the Room Editor filters/effects, SDF support for fonts, plus everything in the actual Prefab Library), tools updates, the GMRT build packages, and all other forms of plugins
    - You'll find it on the Windows menu at the top of the GameMaker window
    - Be aware that a number of these plugins come bundled-in with the GM installer, and so you will always have the version of the prefab which was the current one when your IDE version was made, but you can use Package Manager to update to newer versions if they are released
    - You can specify via the RMB context menu which packages you want to see update notifications for [#11380](https://github.com/YoYoGames/GameMaker-Bugs/issues/11380) and which packages should automatically update themselves [#11381](https://github.com/YoYoGames/GameMaker-Bugs/issues/11381)
    - If there are multiple updates available this will come as a single notification and clicking this will take you to a new "Update All" window [#8625](https://github.com/YoYoGames/GameMaker-Bugs/issues/8625), where you're able to click the one button to queue and update all of the packages [#10297](https://github.com/YoYoGames/GameMaker-Bugs/issues/10297)
    - Package searching supports filtering by short description and by keywords (although not all of the official packages have keywords yet) [#10076](https://github.com/YoYoGames/GameMaker-Bugs/issues/10076)
    - If you want to add your own package source (an npm node), a control is available for when you want your custom node's remote scripts to be performed [#14604](https://github.com/YoYoGames/GameMaker-Bugs/issues/14604) (by default, this will not happen)

- **SVG Support**
    - You can now import and preview some SVG images inside the IDE, and then this image will carry through to your game at build time
    - See these issues for further details on SVG functionality that is/is not supported - [#5895](https://github.com/YoYoGames/GameMaker-Bugs/issues/5895) / [#8276](https://github.com/YoYoGames/GameMaker-Bugs/issues/8276) / [#8275](https://github.com/YoYoGames/GameMaker-Bugs/issues/8275) / [#8274](https://github.com/YoYoGames/GameMaker-Bugs/issues/8274)
    - SVGs containing animations will convert into separate frames in your Sprite (you will get multiple individual frames, not 1 frame that animates in-game) [#10193](https://github.com/YoYoGames/GameMaker-Bugs/issues/10193)
...
- **Extension Options**
    - Added a new system when creating extensions which allows you to declare new fields in a dedicated new "Options" window in the Extension Editor
    ...
    - Added the ability to exclude extension functions from appearing in the Code Editor auto-complete pop-up and also the ability to mark settings as hidden to the “user-facing” options panel on the main Extension Editor window
    - Native-language Android and iOS extensions to access extension options values 
    - And all code injection values are now saved independently per-Configuration also
    - [Android] Also implemented the ability to inject into gradle.properties
...
- **IDE Updater [#3157](https://github.com/YoYoGames/GameMaker-Bugs/issues/3157)**
    - Now, you won't actually see much evidence of this one until the _next_ GameMaker LTS26 release, as there is no newer version to update to yet, but be aware that this release does include the new in-IDE updater for Windows IDE and Mac IDE users (not the Ubuntu Betas!)
    - This new system removes the IDE update notification early on start-up and instead moves it to appear when the Start Page has loaded - you can also do an on-demand check from the new Help menu entry
    ...
    - If you choose "Yes" GameMaker will close and you will (after a short delay) see a simple progress bar while the new version is installed for you
    - Note that, rather than shipping a fixed version of the IDE updater inside the GM installer, GameMaker silently downloads the latest version of the updater when first starting GM after installing it (just like how it gets your GMS2 runtimes)
    - There is no need to routinely do this in order to get new GM versions, but be aware your installed version of the IDE updater _can_ also be changed inside Package Manager - so, if it is ever required to update the Updater in order to see that newer versions of GameMaker are available, you can immediately install the newest updater at that time and then restart GM to check for the IDE update you need

<br>


### Other IDE Features Added Or Changed
...
- **Creating New Assets Now Uses Your Feather Preferences For Naming Conventions [#10477](https://github.com/YoYoGames/GameMaker-Bugs/issues/10477)**
    - The Asset Browser has been changed so that now your initial asset names will use the naming values set in your Preferences for Feather
    - This will only take effect if Feather's "Message Severity" value for rule 2017 is _not_ using the default value of "Ignore" (so you will need to change this in Preferences yourself if you want to use this functionality now)
    ...
    - Also that "Room1" will always be called "Room1", as that asset comes from the base project

- **Game Options Changes**
    - Historically, all product/display names were set as "Created in GameMaker" which, while true, is not a great name for _your_ game, so this has now been changed [#8291](https://github.com/YoYoGames/GameMaker-Bugs/issues/8291)
        - This also lead to issues if you did not update this value in Game Options before releasing your content - for example, GX.games uploads will fail as the name is already taken [#8939](https://github.com/YoYoGames/GameMaker-Bugs/issues/8939) and creating an installer on some platforms will leave your game with an unhelpful install folder name "Created In GameMaker"
    ...
    - Vsync now defaults to ON for all supported targets [#13197](https://github.com/YoYoGames/GameMaker-Bugs/issues/13197) and [#13551](https://github.com/YoYoGames/GameMaker-Bugs/issues/13551)
    - Windows' "Interpolate colours between pixels" setting now defaults to ON, so it matches what the manual has long said and is the same as all the other targets [#2343](https://github.com/YoYoGames/GameMaker-Bugs/issues/2343)
    ...
    - Added new options for toggling the legacy behaviours for various changes made in 2024.8 and later releases, including string-to-number conversion, treatment of "other", and strict audio errors [#8138](https://github.com/YoYoGames/GameMaker-Bugs/issues/8138)
    - Removed the redundant "Generate Mipmaps For Separate Texture Pages" entry, because in recent GameMaker versions you have managed this value in the Texture Groups editor instead [#7045](https://github.com/YoYoGames/GameMaker-Bugs/issues/7045)
...
- **Tile Layers In Rooms: Project Format Changes**
    ...
    - However, this new format is not compatible with older IDEs/runtimes and so accordingly we have introduced a new safeguard in this version, which will block you should you attempt to open the project in an older IDE again (GameMaker will raise the project version automatically to be 1.6)
...
- **Project Load/Room Editor: Filter/Effect Layer Values Are Now Checked For Validity And Fixed Automatically**
    - For situations where you get an in-game error which says "Unknown Function argument 1 invalid reference to (sprite) - requested -1 max is" (e.g. [#8362](https://github.com/YoYoGames/GameMaker-Bugs/issues/8362)), we have now determined this is due to invalid values being set in the Room Editor for your filter/effect layer(s)
...
- **Resource Refactor And Project-Saving Flow Changes**
    - This release introduces a big clean-up to how assets are handled, which should have very minor impact on how you work, but will make future releases far more stable
        - Fixed a very important project-saving bug where if a script's source file had become locked (e.g., by an external process such as source control or Dropbox/Google Drive/OneDrive, etc.) and therefore wasn't able to be read, then it was possible that Feather would incorrectly cause some other assets to be lost (typically Scripts, but not limited to this)
        ...
        - Case-Insensitive Compatibility Mode For The Mac and Ubuntu IDEs
            ...
            - A Preference has been added to allow you to disable these extra renaming checks, as they are quite performance-heavy
...
- **Online/Offline: Better Checks**
    - Changed which "gamemaker.io" websites GameMaker tries to check in order to confirm it is able to reach the internet
    - This will stop silent errors in your ui.log each run
    - Also, if you firewall GM and only allow access to a limited set of our own websites, then this might also fix issues with GM thinking it's offline (however, you should review your permissions settings and consider allowing more sites to be accessed)

- **Anti-Flickering Changes When The IDE Is Running**
    - Added a new Preference to allowing the Windows IDE to toggle your graphics card's FNA3D_D3D11_FORCE_BITBLT setting [#14301](https://github.com/YoYoGames/GameMaker-Bugs/issues/14301), which has been seen in [#14223](https://github.com/YoYoGames/GameMaker-Bugs/issues/14223) to stop flickering for users who have issues using some GSync monitors
        - If you have flickering while using the IDE you can find this in the `General Settings` section as the "Force D3D11 Bitblt" checkbox (the default is OFF)
        - Be aware that the setting is wider-reaching than just GSYNC monitors, so if you have two GPUs in your system or any form of high-refresh monitor and you're seeing flickering while GM is running, then give this setting a go
        - Note, however, that the original bug report talked about flickering when games are running also - that is not affected by this checkbox
...
- **GM Now Creates Crash Dumps Even When It Recovers Enough To Close Itself Down [#11710](https://github.com/YoYoGames/GameMaker-Bugs/issues/11710)**
    - It was realised that in some cases where GM told you there was a problem and so it had to close, because it was able to close safely it didn't then create a crash dump or ask you to send anything to us so we would know about that problem and be able to fix it in a future release
...
- **Code Editor: Struct "Shorthand" Support**
...
- **Code Editor: Defining In-Game Colours Using CSS Values**
...
- **Device Manager Changes**
    - Added a new MQTT server for discovering devices on your network that are running the Opera GX Mobile browser, allowing you to just as easily test your GX.games builds on your phone or tablet as you can on your desktop
        ...
        - You will likely see a new firewall prompt for this the first time you start this new version, so for more information on this functionality and how to use it, please see [our FAQ all about it](https://help.yoyogames.com/hc/en-us/articles/7961446360221)
    - Added support for a dummy iOS device to be created (and then selected in Target Manager and used when building) [#8413](https://github.com/YoYoGames/GameMaker-Bugs/issues/8413)
...
- **Font Editor / Font Support Changes**
    ...
    - Inspector: [Fonts] Changing properties did not update the font at runtime, had to regenerate manually to see a change [#11926](https://github.com/YoYoGames/GameMaker-Bugs/issues/11926)

- **Go To (Ctrl+T): New Additions**
    ...
    - Go To is now re-enabled when on the Start Page, but it will only search inside Preferences
...
- **Image Editor: Support For Pasting As A New Layer**
    ...
    - However, we also added the ability to turn your current selection into a custom brush even when paste-to-layer is enabled - the default hotkey for this is Ctrl+B (Cmd+B in the Mac IDE)

- **Inspector Changes**
    - Included Files are now inspectable and so settings can be quickly modified after single-clicking the Included File
    - Extension Options values are now inspectable [#5345](https://github.com/YoYoGames/GameMaker-Bugs/issues/5345)
    - You can now set a friendly name for each variable in your Objects' Variable Definitions window [#13141](https://github.com/YoYoGames/GameMaker-Bugs/issues/13141)
        - This can of course be a useful helper for yourself; however, the real benefit is that these friendly names will now appear when that Object is used in a prefab and _someone else_ imports that prefab into their project, as now they can get your helper text also - which can really help them identify much more quickly/easily what each variable is used for
...
- **Licensing Changes**
    - Sessions are now simply "one for the host machine" instead of "one per-IDE install on that machine"
        - This means if you're not just using this LTS, but you also have a Monthly or a Beta install (or Steam and non-Steam installs) on a single physical machine, then this will only result in one licensing session
...
- **Marketplace Integration Has Been Removed [#9304](https://github.com/YoYoGames/GameMaker-Bugs/issues/9304)**
    ...
    - Also be aware of these "extra" bits that are to do with Marketplace functionality, but appear elsewhere in the IDE:
        - Startup: GM no longer downloads your asset manifest [#9906](https://github.com/YoYoGames/GameMaker-Bugs/issues/9906)
...
- **Room Editor Changes**
    ...
    - "Select from Any Layer" no longer turns off whenever selecting something on a Tile or Path layer [#6924](https://github.com/YoYoGames/GameMaker-Bugs/issues/6924)
        - Previous GM releases would always turn this setting off, as Path and Tile layers were handled differently, but now the setting will stay enabled; however, a new Preference been added for if you want to keep the old behaviour (defaults to OFF)
    ...
    - Now shows the current grid location's X/Y values on the status bar [#2851](https://github.com/YoYoGames/GameMaker-Bugs/issues/2851)

- **Sequences: Text Tracks**
...
- **Sound Editor/Inspector Changes**
...
- **Sprite Editor: Float Values For Stretched Frames [#11822](https://github.com/YoYoGames/GameMaker-Bugs/issues/11822)**
    - Float values are now allowed in stretched frames
    - Also, you can now set these values via the new text field, which might often be easier to type precise values in rather than dragging the frame edges with your mouse

- **Texture Groups Editor: Can Now Manage All Sprite Types**
...
- **Visual Editor: Changes For Collision Actions**
    - Updated the various Visual nodes relating to collisions, so that they can now accept arrays and also tilemap IDs, so that the new tilemap-based collisions feature can be used in Visual editors also

- **Workspaces: Pinch-To-Zoom Has Been Re-Implemented**
...
- **Misc Other Tweaks To UI Functionality**
    ...
    - Asset Browser: Restored the ability to have multiple AB windows at once (like the old Resource Tree allowed) via clicking the Window > Asset Browser menubar command each time you want a new window to open
    - Asset Browser: Removed the independent dialog to customise a group's colour when right-clicking on its icon area - you could always edit the colour on the item’s main right mouse button menu anyway, so now right-clicking anywhere on the whole row will always open the full menu
    - Menubar: Added new "Show/Close All Windows" entries to the Layouts section, so these are no longer only on the Workspace right-click context menu
    - Preferences: “Default Audio Device” has been removed in favour of just using the OS's current default playback device [#9256](https://github.com/YoYoGames/GameMaker-Bugs/issues/9256)
    - The Preference where you could pick a specific audio device to use in GameMaker has been removed and instead the IDE now always uses the operating system’s default output device
    ...
    - Target Manager: You can now double-click any device name to open Device manager and edit it - you now don't have to click the pencil button every time

<br>

### Other Build Features Added Or Changed

{% include important.html content="Your build machine will require some extra tools and settings inside GameMaker to support the changes listed below, so please see [the Required SDKs page](https://github.com/YoYoGames/GameMaker-Bugs/wiki/2026.0.0) and follow the setup guides linked-to there **before** attempting any builds or sending any bug reports!" %}

- **Optimised Calling Of Functions (Introduces Changes To Inheritance!) [#10202](https://github.com/YoYoGames/GameMaker-Bugs/issues/10202)**
    ...
    - You now need to do one of either:
        - add `self.` before functionA calls functionB
        - change the functionA declaration to be a method (so, rather than `function functionA(){}` use `functionA = function() {}` now)
    ...
    - A small project which shows the code changes required before-and-after for the `self.` route above can be found in [this comment on the original issue](https://github.com/YoYoGames/GameMaker-Bugs/issues/10202#issuecomment-2792377582)
...
- **Support For Not Including Unused Assets In Your Export Packages**
    - You can now enable whether during builds GameMaker should automatically ignore any unused assets in your projects, so that these assets are not added into your game packages (the project itself is obviously not modified and the unused assets are safely left alone on your computer)
...
- **Preferences For Path/Command Line Path Values Are Now Verified During Builds**
    - If you try to build for a target which you have not correctly configured inside Preferences, you will now more reliably get an error message telling you which setting needs to be fixed (and again the build will be failed properly
    - Similarly, if a required path for command line builds was not supplied when triggering the build this could previously result in a silent crash of the build tools, rather than a useable error message saying that you needed to supply the required path (and which specific path was at fault), but now all of the paths are checked before the build is started and if issues are found the build will be correctly stopped and you will get an error message asking you to supply the missing info

- **More Compilation Errors Are Shown During Builds**
    - A number of errors and warnings that the AssetCompiler And Igor tools wrote to your Output window log were not using the correct formatting to make the IDE also show the lines in the Compiler Errors panel for ease of finding them later, but now these logging messages have all been standardised and so will show up correctly [#10654](https://github.com/YoYoGames/GameMaker-Bugs/issues/10654)
    ...
    - Compile Errors for a problem with your Variable Definitions values will now open the correct place and therefore highlight the problem much more clearly [#13583](https://github.com/YoYoGames/GameMaker-Bugs/issues/13583)

- **{macOS, iOS, tvOS] M-Series CPU Macs Are Better Supported**
    - Updated the XCode project to allow VM and YYC builds to run on simulators launched on an M-series Mac (Intel Macs worked fine already)
    - Changed the Application Oven to also support M-series Macs (you shouldn't see any difference when building projects, etc., but this means the app with now run natively on these Macs)

- **In-Game Sprites Now Use QOI Image Format**
    - Games now use a modified version of the QOI image format with BZ2 compression, which is smaller than PNG (we saw in our testing that game packages are now around 80% of the size they would be in 2.3.7) and quicker to decompress
    ...
    - The Texture Group Editor has a new "Texture Group Format" drop-down where you can control whether QOI images should also be compressed or not - or indeed if you actually prefer to stick with the old .png output method
    - Be aware that by default, GM will use QOI+BZ2 during export package builds and only QOI when doing a run/debug - if you want faster loading in your final packages, use QOI only; if you want smaller package sizes and lower memory usage continue to use QOI+BZ2

- **Extensions Support Has Several New Abilities**
    - Added the ability to pass GML struct keys to Extensions
    - You can now create a "MacFrameworks" folder in your project source and any Framework .zip files in here will be added automatically during your macOS YYC builds (not VM) [#10429](https://github.com/YoYoGames/GameMaker-Bugs/issues/10429)
    - Similarly, this release adds the ability to embed a Framework in a generated iOS app
    - You can now use `pre_project_step.bat` and `post_project_step.bat` (and their .sh equivalents) in your extensions [#11017](https://github.com/YoYoGames/GameMaker-Bugs/issues/14088)
        - When your project is being built, all extensions are scanned to see if they contain these files and they will be performed if found
    ...
    - Extensions which require XCode projects to support C++/ObjC++ and Swift interop are now supported [#12719](https://github.com/YoYoGames/GameMaker-Bugs/issues/12719)
    - An extension's version number is now exported so that using `extension_get_version()` in-game will return this and also so that can be checked in batch files/scripts using the environment variable `GMEXT__Version`
    - And `extension_exists()` has been added so you can now query if an extension is available in-game before acting accordingly
...
- **New Launch Options When Doing Builds**
    ...
    - A series of new options to launch builds and clean your cache areas are also now available in the Build menu, which will help reduce build times and allow you to clean only the specific parts of your project you need to
...
- **Commandline Builds: Added The Ability To "Just Build" [#9367](https://github.com/YoYoGames/GameMaker-Bugs/issues/9367)**
    - Added a new "Compile" option, which simply performs the build and then stops - no zipping/packaging or running the game, etc.

- **Batch Files Can Also Be Called During Clean Operations [#8695](https://github.com/YoYoGames/GameMaker-Bugs/issues/8695)**
    - You can now create your own batch and shell scripts for `pre_clean_step.bat` and `post_clean_step.bat` (and their .sh equivalents)
    - These pre/post scripts can also be added to your extensions to add similar functionality there

- **Use keywords as variables**
    - It is now possible to use GameMaker's in-built keywords (e.g., `then`, `do`) as variable names
    - Please note that Feather/Diagnostics in the two Code Editors still marks this as a coding error, but the project will build and run with no problems
...
{% include important.html content="Note that, especially the consoles, this is just a summary of the key information you need to be aware of, but there are a huge number of additional changes as part of the bug reports lower down this page - again, please see [the Required SDKs page](https://github.com/YoYoGames/GameMaker-Bugs/wiki/2026.0.0) and follow the setup guides linked-to there **before** attempting any builds or sending any bug reports! (For consoles, see their own wikis - which you can easily reach via [the wiki front page](https://github.com/YoYoGames/GameMaker-Bugs/wiki/).)" %}

- **Android Build Changes**
    - Google Play requires all submissions as of Nov 2025 to use 16kb-aligned page sizes - this release brings that support to LTS [#10894](https://github.com/YoYoGames/GameMaker-Bugs/issues/10894)
    ...
    - You can now enable Edge-to-Edge display and also allow control of the display layout via new settings in Game Options-> Android-> Graphics  [#8186](https://github.com/YoYoGames/GameMaker-Bugs/issues/8186)
    ...
    - Removed some redundant "setting vis flags" debug output, so that logs are a bit clearer
...
- **HTML5 Build Changes**
    - As the HTML5 runner has now been open-sourced, you can use the new obfuscation settings in `Preferences > Platform Settings` to help track down issues in your games more easily: Obfuscate (or not), Pretty Print, Remove Unused Functions, and Verbose (or not)

- **PlayStation 5 Build Changes**
    - You can now enable Low-Energy Mode in your Game Options and it will be supported on the console (defaults to ON if you create a new project or param.sfo file)

- **Switch Build Changes**
    - Implemented Nintendo's native crash-reporting system 
        - Now, in-game errors will trigger the console's own error system, rather than GM's usual one, and so they will also create a crash dump which can be sent to you by your players/testers for you to analyse
        - Be aware that the above may require changes to your metadata next time you submit
    
- **Ubuntu Build Changes**
    ...
    - There is a new option to build AppImage packages when building for Ubuntu (you can still build a .zip also)
...
- **Windows Build Changes**
    - Added support for direct polling of the mouse cursor location [#13394](https://github.com/YoYoGames/GameMaker-Bugs/issues/13394)**
...
- **Xbox Build Changes**
    ...
    - `xboxone_get_achievement()` can now be called to query the progress of the chosen achievement
    - Added 120Hz support for Xbox Series X/S
    - Please see the Xbox docs for more information on all of these

<br>

### Other In-Game Features Added Or Changed

{% include important.html content="Note that this is just a summary of the key information you need to be aware of, but there are a significant number of additional small changes as part of the bug report titles lower down this page." %}

- **[Android, iOS] New Event Added For "Device Layout Changed" [#8186](https://github.com/YoYoGames/GameMaker-Bugs/issues/8186)**
    - A new "Async System" message is sent whenever the display layout changes
...
- **Arrays: Added a lot more functions for searching/manipulating**
    - Because there are a lot of them, we can't list them all here, but there have been a number of new functions such as `array_all()`, `array_any()`, and `array_filter()`, which have been added to allow you to work with arrays in a more extended way
    - Most of these new functions leave the original array(s) untouched and instead return you a new array containing the end results of the requested operation; however, there are also some new  `..._ext()` versions also, for if you do want to modify the original array - as a couple of examples: `array_unique_ext()` and `array_reverse_ext()`
    - The return value from all of these  `..._ext()` functions is a real which gives you the new size of the modified array
    - Of course, more on all of these new functions can be found in the manual

- **Audio: Functions Now Have Strict Error-Checking And Catch More Coding Issues**
    - This release will now more reliably catch when there are errors with audio playback due to invalid assets or arguments, etc., in your code [#7285](https://github.com/YoYoGames/GameMaker-Bugs/issues/7285)
    ...
    - And as of [#10198](https://github.com/YoYoGames/GameMaker-Bugs/issues/10198) this will allow you to disable more types of coding mistakes (although, again, review carefully as to whether you should actually just fix the real problem instead of turning off the error)

- **Audio: Changes For Calculating An Instance's Final Pitch**
...
- **Audio: Create Loop Points During Playback**
...
- **Audio: Lots Of Misc. Other Additions And Fixes**
    - `audio_play_sound...()` now have new optional arguments for "gain", "offset", "pitch" and "listener_mask" - which means these values will reliably take effect immediately when the sound plays
        - These values are also remembered and reused each time the instance is played again or resumed after pausing
    - Two new audio falloff models have been added for use with `audio_falloff_set_model()`:
    ...
    - Fixed various issues where voice-level and source-level audio properties are conflated within the runner and so perhaps were always played with the wrong values in previous releases
...
- **Audio Filters/Effects Support**
    ...
    - `audio_bus_create()`, `audio_effect_create()`, `audio_emitter_bus()`, and `audio_emitter_get_bus()` are all new and allow you to call your choice of the following effects:
        ...
        - Peak EQ filter (AudioEffectType.PeakEQ) - A peak EQ filter creates a peak or a dip in the spectrum by a specified gain around a specified centre frequency
            - Each of the above filter functions have the same parameter set:
        ...
        - Tremolo - a gain modulation effect, with modulation being driven by a separate low-frequency oscillator (LFO) for each channel. This allows for effects from gentle gain fluctuations all the way up to stuttering with  ping-pong panning
...
- **Changes To `Noone` Being Returned By Some Functions**
    - When reviewing [#9374](https://github.com/YoYoGames/GameMaker-Bugs/issues/9374) we found several other places where `noone` was being passed back as the number -4 rather than an instance handle
    - These have all been changed to return an instance reference of -4
    - This may affect any code you have in your projects where you are checking for `is_number()` or `is_handle()` rather than looking for `noone`
    - Note that most of the locations changed were to do with collision code - where `noone` is passed back when nothing is found

- **Clipboard Functionality: Supported In More Targets**
    - Added support into Android, GX.games, HTML5, iOS and macOS for the existing clipboard functions already supported on other platforms
    - Note that tvOS is not included in these additions because it does not actually support any clipboard functionality
    - However, with HTML5 it is worth noting that Firefox does not allow pages to access the clipboard via JavaScript, so that browser will not work by default (you need to install browser extensions), and that Internet Explorer is too old to support the standard used and will not work at all

- **Collisions: Changed Bounding Box Values To Be Floats And Added A Compatibility Mode**
    - There is quite a substantial change to collisions, as bounding box values are now floats rather than ints - this increases accuracy and fixes a number of long-standing bugs (see below for some of the bugs fixed), but you should test your games carefully, as you may now need to make changes to your code in response to this fix
...
- **Debug: Overlays In-Game Are Now Far More Feature-Rich**
    - We have incorporated Dear ImGui into the runner to have a better and richer debug UI experience
    - The old FPS overlay is now replaced with an improved graph showing changes over time and a stacked snapshot of the current frame as well
    ...
    - There are a lot of functions for creating your own custom controls/layouts, via creating sections and then adding buttons/sliders/text fields, etc., into them, so please do see the manual for all of this
    - Mouse, keyboard and gamepad input are all supported, so you can interact with the various windows and controls as you wish
        - Note that you can toggle whether or not you want any gamepad support, but also if you want only one specific gamepad to interact or to allow all pads [#11705](https://github.com/YoYoGames/GameMaker-Bugs/issues/11705)
    - `dbg_add_font_glyphs()` adds a TTF font to be used in the debug GUI
        - The font should be added to your Included Files. The size of the font can be specified in pixels and a font range can be selected (glyphs must be present in the given TTF font) * -1 - Default Latin range, * 0 - Greek, * 1 - Korean, * 2 - Japanese, * 3 - Chinese Full, * 4 - Chinese Simplified Common, * 5 - Cyrillic, * 6 - Thai, * 7 - Vietnamese

- **Debug: Functions For Automating Inputs [#8409](https://github.com/YoYoGames/GameMaker-Bugs/issues/8409)**
    - `debug_input_record()`, `debug_input_save()`, and `debug_input_playback()` functions have been added to allow for developer test-automation
    - Note that these are only intended for use when testing your own games - not as part of released games!

- **`debug_event("ErrorOnWarning")` Turns Warnings Into Code Error Dialogs Which Include A Callstack [#4118](https://github.com/YoYoGames/GameMaker-Bugs/issues/4118)**
    - In older runtime versions you would simply get a debug warning for a number of coding errors and there was no real information on how this problem had been encountered/where to look in your code in order to go about fixing it
    - As some examples given on the issue show, this is messages like "Grid Index Writing Out Of Bounds" or "Error: Index did not map to an existing audio asset" which show up in your Output window
...
- **`gml_pragma()` Support For Always Compiling Assets With Your Specified Tags [#8313](https://github.com/YoYoGames/GameMaker-Bugs/issues/8313)**
    ...
    - As an example, if I have an IDE tag called "AlwaysCompileMe" and I have assigned that to some assets, I could now type : `gml_pragma("MarkTagAsUsed", "AlwaysCompileMe");`

- **Particles: New Functions For Spawning Systems Created In The IDE But With Customised Values [#12150](https://github.com/YoYoGames/GameMaker-Bugs/issues/12150)**
    ...
    - Also, there is an accompanying new function to delete the system when you're done with it

- **Physics: Create Shapes At Runtime [#9472](https://github.com/YoYoGames/GameMaker-Bugs/issues/9472)**
...
- **[macOS/iOS] "ProMotion" Support On Apple Devices**
    - Added support for adaptive display refresh rates on the specific devices which support this "ProMotion" feature (note, this is only a handful of newer device models at this time)
    - The game will detect and handle this with the device for you - there are no new functions for this feature

- **`random_set_seed()` Now Allows Greater Range Of Seed Values [#3006](https://github.com/YoYoGames/GameMaker-Bugs/issues/3006)**
...
- **Rollback Multiplayer Chat Functionality**
    - New functions and a new Rollback event have been added for providing chat functionality to your players

- **`room_get_info()` Returns Info About The Room You Specified**
    - This function returns a struct that contains all of the data that has been requested, but be aware a room can be very large and so this can take a long time to complete, so some data can be omitted if you want to help speed it up.
...
- **Rooms/Sequences: Support Persistent Alignment Values For Text Instances [#11637](https://github.com/YoYoGames/GameMaker-Bugs/issues/11637)**
    ...
    - For more, including a sample .yyz file which shows the new functionality, see the issue
...
- **SDF ("Signed Distance Field") Font Support**
    ...
    - Fonts can be set as supporting SDF either via a checkbox in the Font Editor or "upgraded" later on in-game using the function mentioned below
    - There are two new GML functions to enable\disable rendering using SDFs:
        - `font_enable_sdf()` allows enabling or disabling SDF rendering for that font
        - `font_get_sdf_enabled()` queries whether SDF rendering is currently enabled for that font
    - Also there are two new functions for controlling how the SDF will look:
        - `font_sdf_spread()` sets the "spread" value for the specified font, which represents the distance the SDF extends from the edge of each glyph and is measured in pixels (the value can vary from 2 to 32 pixels). Larger spread values allow for custom shader effects that require bigger radii (such as glows) and smaller spread values mean that each glyph takes up less space on the texture page and also improves rendering precision - note that this "spread" value will only be used for FreeType fonts added via `font_add()` and therefore does not apply when running on HTML5
        - `font_get_sdf_spread()` returns the spread value for the specified font (or -1 if the font does not exist)
    - Improves the packing of glyphs for .ttf fonts added using `font_add()` on all platforms apart from HTML5, as excessively large glyphs in the font will not affect the packing unless they are actually drawn - and also GameMaker does not crop the edges of the SDF when drawing each glyph

- **Sequences: Properties Array Entries Can Now Be Set Individually [#12723](https://github.com/YoYoGames/GameMaker-Bugs/issues/12723)**
...
- **Spine: Additional New Functionality**
    - Added support for Spine's "Mixed-Skin" functionality
    - "Spine Collision Mesh" option settings added into the Sprite Editor
        - Previously, GameMaker would inconsistently apply one of _either the bounding box and/or collision mask from a Spine collision mesh_ or _the precomputed mask from the Sprite Editor_, depending on different code paths executed in the game
        - This new option allows you to explicitly choose (on a per-sprite basis) to consistently use Spine collision meshes OR GM collision masks in all code paths
    - Pre-multiplied alpha control functions have been added [#12070](https://github.com/YoYoGames/GameMaker-Bugs/issues/12070)
        - By default, GameMaker will automatically set the appropriate blend mode to correctly draw Spine sprites that use pre-multiplied alpha, as long as the current blend mode is set to the default (if the blend mode is set to anything else it's assumed that you have set it for a reason)
        - However, we've added a couple of functions to disable this again if that behaviour does not work for your needs:
            - `draw_enable_skeleton_blend_override()` - "true" enables the override (new behaviour) and "false" to use the previous behaviour (the default setting is true)
            - `draw_get_enable_skeleton_blend_override()` returns true/false if the override is enabled or not
    - There are also a number of new/changed functions you can use:
        - Added new optional "looping" and "position" arguments to `skeleton_animation_set()` and `skeleton_animation_set_ext()`
        - `skeleton_animation_get/set_position()` will return/set the position of the track you specify as a percentage, not as a specific frame
        - `skeleton_animation_is_looping()` and `skeleton_animation_is_finished()` can be used to check the current status
        ...
        - `skeleton_attachment_create()` will now return an error if you try to reuse an attachment name without destroying it first
...
- **`sprite_add_ext()` Added For Loading Sprites Asynchronously**
...
- **Statics: Changes To Behaviours**
    - Static initialisation no longer runs multiple times [#7888](https://github.com/YoYoGames/GameMaker-Bugs/issues/7888)
    ...
    - Improved handling of static values so it is more intuitive, by introducing the idea of a static chain when a constructor inherits from another
        ...
        - `is_instanceof()` returns true if struct has anywhere in its static chain
        - `static_get()` returns the static struct for a given struct or function, or "undefined" if it has no static
        - `static_set()` sets the static for a struct to point at another struct (Note: You should be very careful using this one, as it is provided for deserialisation use-cases only)

- **Structs: Extended And Introduced Various Functions**
    - `struct_foreach()` calls the provided function on each member of the struct
    - `struct_get_from_hash()` gets the value of a struct member using hash obtained from `variable_get_hash()`
    - `struct_set_from_hash()` sets the value of a struct member using hash obtained from `variable_get_hash()`
    - `variable_get_hash()` returns a hash value you can then use with the above two functions
    - `variable_clone()` returns a clone of the value that is passed - but note that it does not clone data structures; rather, it just copies them as numbers and the "depth" value defines the number of levels to go through
    - Note also that `struct_get()`, `struct_get_names()`, `struct_set()`, `struct_exists()`, `struct_remove()`, and `struct_names_count()` have all been added also, but these are simply aliases to the existing functions you would get if you typed `variable_` before each of them and the new names are simply to save you keypresses in future

- **`surface_create()`: New Surface Formats Supported**
    - In all previous releases, `surface_create()` did not allow you to specify the format of your new surface and instead always used R8G8B8A8, but now you can call one of several constants as a new optional argument to `surface_create()` in order to control this
...
- **WebSockets: New Additions**
    - Secure sockets are now supported on all platforms except for GX.games
        ...
        - `network_connect()`'s second argument now supports providing a `wss://` URL for use with _ws or _wss sockets
    - Note also that the above change also added some basic parsing to support paths on the end of the socket URL value (e.g. `foo.com/pathtomysocket`)
    - `network_send_raw()` now has an optional "options" parameter which can be set to "network_send_binary" or "network_send_text" types
        ...
        - Similarly, the matching `async_load` map has a new "message_type" field for all received WebSocket messages, which will also be set to either of the above types

- **Zip/Unzip: New Asynchronous Functions**
    ...
    - `zip_add_file()` adds a copy of the given file to a to a Zip structure - for nesting, include slashes in the path (e.g. "myFolder/test.txt")
    - `zip_save()` finalises the Zip structure (meaning that no new files can be written to it) and saves the archive to the given path
    - Note that `zip_save()` is performed asynchronous and will trigger an event on success or failure

- **Misc. Other Additions To In-Game Functionality**
    - Buffers, Vertex Buffers, Vertex Formats, and Surfaces now have typed handles
    - Vertex buffers can also now be modified after they have been created and filled
    - Decoupled audio-decoding threads from the audio context [#7055](https://github.com/YoYoGames/GameMaker-Bugs/issues/7055)
    - Strings for handles to an asset now use the name of the asset rather than the number [#9053](https://github.com/YoYoGames/GameMaker-Bugs/issues/9053)
    - Added two new functions for controlling layer sequence image blend and alpha values: `layer_sequence_alpha()` and `layer_sequence_blend()` [#14404](https://github.com/YoYoGames/GameMaker-Bugs/issues/14404)
    - Added compile-time macros for `__LINE__`, `__FILE__`, and `__FUNCTION__`, so your games can show this debugging info if you require it
    ...
    - `audio_bus_get_emitters()` can now get an array of all emitters linked to that bus
    - `audio_bus_clear_emitters()` can now relink all of a bus's emitters
...
- `buffer_get_used_size()` returns the actual usage size of the specified buffer, so you can see how much space is left in your buffer, etc. [#9298](https://github.com/YoYoGames/GameMaker-Bugs/issues/9298)
    ...
    - `GM_runtime_type` is a new constant for determining if a game was built with the GMS2 runtimes or GMRT [#9055](https://github.com/YoYoGames/GameMaker-Bugs/issues/9055)
    - `instance_create_...()` now allows "undefined" as the `var_struct` optional parameter value [#12218](https://github.com/YoYoGames/GameMaker-Bugs/issues/12218)
    ...
    - `lin_to_db()` and `db_to_lin()` - convert linear gain to decibels and vice-versa
    - `matrix_inverse()` inverts whichever matrix which you supply to the function [#2960](https://github.com/YoYoGames/GameMaker-Bugs/issues/2960)
    - `method_call()` - you can now pass in the method and its args array, plus an optional offset and length (both of which can be negative, so you count backwards)
    - `particle_get_info()` allows you to access particle system information at runtime - the returned values are broken down into 3 sections: a Particle System Info struct, an Emitter Info struct, and a Particle Type info struct
    - `physics_debug()` enables\disables physics errors which were previously being silently ignored (sample project on the linked issue) [#8997](https://github.com/YoYoGames/GameMaker-Bugs/issues/8997)
    ...
    - `script_execute()`/`script_execute_ext()` can now call constructors [#7995](https://github.com/YoYoGames/GameMaker-Bugs/issues/7995)
    - `sprite_add()` and `sprite_replace()` can now use data URLs in to add and replace your sprites in game[#9344](https://github.com/YoYoGames/GameMaker-Bugs/issues/9344)
    ...
    - `surface_get_format()` returns the format of the given surface (all possible formats are listed in the manual page)
    - `tileset_get_info()` works the same as spite/font_get_info() and returns a struct containing lots of information about this tileset (or "undefined" if the argument is not a valid tileset)
    - `window_minimise()` and `window_restore()` functions have been added for Windows, macOS, and Ubuntu [#7102](https://github.com/YoYoGames/GameMaker-Bugs/issues/7102) (Although, be aware that macOS will not respond to the restore function, as games are paused by the OS when they lose focus)

<br>

#### Per-Platform Changes

- **Android In-Game Changes**
    - Updated the audio backend to be AAudio via Oboe [#14513](https://github.com/YoYoGames/GameMaker-Bugs/issues/14513)
    ...
    - Support has been added for slightly older APIs (pre-35) to support edge-to-edge displays when there are cut-outs (`windowLayoutInDisplayCutoutMode`) [#8186](https://github.com/YoYoGames/GameMaker-Bugs/issues/8186)
        - In API 35+, this is always enabled in the Android tools, so this setting only affects older versions
...
- **GX.games In-Game Changes**
    - GX.games now uses the same audio output system used on most other native platforms, so you should find better performance
        - Note that this affects all sound playback, including video support
    - Device gyro/tilt functionality is now supported on Android and iOS mobile devices [#9267](https://github.com/YoYoGames/GameMaker-Bugs/issues/9267) (example project on the issue)
    ...
    - Added a new capture device for use by the runner, so now the microphone/recording functions which have worked on other targets for a long time now work on GX.games also
    ...
    - You can now capture the feed from a camera and open it with `video_open()` in your games [#9229](https://github.com/YoYoGames/GameMaker-Bugs/issues/9229) (example project on the issue)
    ...
    - Added a new "save as .zip" output type when clicking Create Exe
        - This package type is intended for those who want to use the WASM package that the GX.games target creates for you, but on your own website rather than on the GX Community (and so you need to have webhosting and a suitable web page already)
    ...
    - Added the ability to share screenshots and URLs [#13701](https://github.com/YoYoGames/GameMaker-Bugs/issues/13701)
    ...
    - Removed "QR" and "Console" buttons are now removed when exporting using the Create Exe options (they do still appear for run/debug, as you'd expect) [#12183](https://github.com/YoYoGames/GameMaker-Bugs/issues/12183)
    ...
    - Extensions no longer pass in `self` and `other` as arguments [#12187](https://github.com/YoYoGames/GameMaker-Bugs/issues/12187)
    ...
    - Added support for virtual keys

- **HTML5 In-Game Changes**
    - Changes to Step event and instance-creation behaviours
        ...
        - This fixes a number of projects that have had issues on HTML5, and means HTML5 now has more behaviour parity with the other platforms, but may require you to refactor your code now if you relied on these incorrect  behaviours in your games
    - HTML5 now records audio at the same sample rate as other platforms, so you no longer need to code for different-sized buffer lengths if using this target also
    - Added support for virtual keys
    ...
    - `other` behaviour now correctly always uses the Game Options value [#10885](https://github.com/YoYoGames/GameMaker-Bugs/issues/10885)

- **PlayStation 5 In-Game Changes**
    - PS5 games can now see and read PS4 saves for the same title (note that this is not supported going from PS5 to PS4)

- **Switch In-Game Changes**
    - NPLN support added
        ...
        - There are a large number of `switch_matchmaking_session_...` and `switch_npln_...` functions added for this feature, so please do see the Switch manual docs for all the info here
    ...
    - Also added a new function for NEX matchmaking to mirror the new NLPN description function `switch_matchmaking_session_update_description()`, but for NEX games
    - Implemented new functions to create blocking/non-blocking network requests as required

- **tvOS In-Game Changes**
...
- **Windows In-Game Changes**
    ...
    - `window_set_showborder()` allows toggling borderless window mode on-demand
    - New `debug_event()` commands allow various GPU settings to be controlled in-game. These new events are:
        ...
        - Should you find that your game still exhibits some form of lag in this release, please do use [this test project (also attached in the comments on #14046)](https://github.com/user-attachments/files/25718363/LagTester.zip) on the machine which shows the problem and then file a new bug report saying which values cause/fix the issue on your PC (don't send us a different sample or your full game - just use this premade test project and tell us the result)

- **Xbox In-Game Changes**
    - You're now able to load UWP game saves using `xboxone_set_savedata_uwp_compatibility()` - this function must be called before the game performs any savedata-related file operations (Note: this function is only to be used in projects which are updating an already-released UWP title to use GDK instead)
    - Video playback on Xbox has always worked slightly differently to the other consoles: on Xbox, videos were always drawn to fill the currently-set surface, whereas on the other consoles a new surface is created and which the video is decoded to, and it's then the game's responsibility to draw that surface to wherever it's required to be displayed - this release changes Xbox so that it matches the other platforms, and accordingly any code you have for video playback on Xbox will need to be reviewed now

<br>

### Other Docs Features Added Or Changed
...
- Asset Browser: [Mac IDE] GM no longer goes unstable when cancelling the conversion of a 2.2.5 project which contains an asset you are currently performing Add Existing on
...
- Licensing: GM no longer crashes when clicking Log Out if your user data folder and um.json file were already deleted by you whilst GM was still open
...
- Project Save: GM no longer goes unstable or corrupts your project when attempting to import unreadable .ogg files
...
- Stability: Fixed a crash when Bluetooth headphones temporarily lost connection with laptop [#11857](https://github.com/YoYoGames/GameMaker-Bugs/issues/11857)
...
- Startup: [Mac IDE] Fix for how system fonts are detected and loaded by GM
...
- Startup: [Mac IDE] Fixed that macOS assumes arguments on a startup shortcut’s "open" command are always files to load and crashes GM while trying to find them
- Startup: [Mac IDE] GM no longer crashes on launch if the IDE language value is set to "Simplified Chinese"
- Startup: [Mac IDE] GM no longer sometimes shuts down when "Checking Network..." shown on Splash Screen if your Mac is offline at that time
- Startup: [Mac IDE] Steam IDEs launched on M1 Macs no longer fail to pass Steam params to the login API
- Startup: [Windows IDE] GM no longer crashes on launch if required manual RSS feed files are locked by another GM instance which is also starting up at the same time (i.e., if you tried to really quickly open multiple copies of GM)
...
- Startup: Fixed that after updating to new versions of GM your language preferences were sometimes not retained if you were logging-in using SSO (or perhaps you would momentarily see the wrong language and then it would fix itself when you reached the Start Page)
...
- Startup: GM no longer crashes on receiving http error "(417) Expectation Failed." from a proxy server stopping a user from logging-in (you will still need to resolve your proxy server issue, but this fix means GM remains stable and tells you about the problem)
...
- Startup: Stopped recreating GM's default "Paths" folders before checking if the values have been customised by you in Preferences already
...
- AnimCurve Editor: Ctrl+Mousewheel now zooms to a point centred on the mouse pointer
...
- AnimCurve Editor: Fixed the AnimCurve library to no longer display a grey overlay on any presets section that overlaps into the type section [#12341](https://github.com/YoYoGames/GameMaker-Bugs/issues/12341)
...
- Asset Browser: Fixed that the Room Manager "Collapse All" context menu command did not work unless you clicked in empty space
...
- Code Editor (Visual): Added nodes for the Get/Set/Use Rollback Player Preferences functions
...
- Docs Integration: Fixed that the Help menu "Knowledge Base" link incorrectly opened an old subcategory page, rather than the actual main landing page
...
- Feather: `network_send_raw()` helper text has now been updated to match that this function allows 4 arguments
...
- Feather: Functions expecting a bool argument type (e.g. physics_joint_xxxx_create) will now determine if the value being passed-in is suitable before showing an error or not - if the value can be converted to a bool, this will be allowed
...
- Filewatcher: Improved the messaging/handling of the "too many changes" error, by quickly confirming if the entire project folder has been lost or not and giving a more useful message if it was deleted whilst GM was open
- Filewatcher: Reloading external modifications to an asset no longer causes GM to say it was "lost" and move the asset to the root of your Asset Browser
...
- Game Options: Fixed that the GX.games section's "Edit Game on Opera" button did not actually open the browser for some users
...
- Image Editor: Copying 256-colour image data from outside GM and pasting it in no longer flips the red/blue values
...
- Image Editor: No longer continues showing any cached content for sprites which have been removed while GM was closed
...
- Installer: [Mac IDE] The correct Dock icon is now shown on initial launch of GameMaker after installation
...
- Installer: [Windows IDE] Fixed that the "missing freetype.dll" error dialog shown if GameMaker cannot start still said "GMS2"
...
- Localisation: [BPT/RU] Fixed that "Switch" (as in the Nintendo console name) was mistakenly translated on multiple dialogs
...
- Preferences: "Import From Your Other GM Installs" support has been added
...
- Preferences: Platform Settings > GX.games now has options for controlling the Webserver (exactly as HTML5 has)
...
- Project Load: Attempted fix and also a lot more logging for GM 2024.14 crash when loading projects containing sounds/importing new sounds on my PC [#12485](https://github.com/YoYoGames/GameMaker-Bugs/issues/12485)
...
- Project Save: Deleting an Included File no longer leaves a reference in the .yyp [#10778](https://github.com/YoYoGames/GameMaker-Bugs/issues/10778)
- Project Save/Load: Deleting an extension function no longer breaks loading projects [#10416](https://github.com/YoYoGames/GameMaker-Bugs/issues/10416)
- Project Save/Load: Project names containing emojis no longer cause Game Options read errors [#10581](https://github.com/YoYoGames/GameMaker-Bugs/issues/10581)
...
- Room Editor: Colour Tint effect type's colour picker is now one which does not show alpha settings - the alpha was correctly being ignored in older GM versions, as this is not part of the effect, but the picker implied it should have worked
...
- Sprite Editor: [Mac IDE] Restored the old behaviour that files dragged onto the image window now replace the existing sprite content as you'd expect (and as the WIndows IDE still does), rather than creating a new sprite like GM did in the last few releases
...
- Target Manager: There's a new Preference for changing the default build platform used when opening/creating a new project - which can be found in `Preferences > General Settings`
...
- UI/UX: [Mac IDE] Fixed that you could not open several context menus inside editors if the "IDE Language" Preference is set to "Simplified Chinese"
...
- Webserver: `Preferences > Platform Settings > HTML5` values are now being respected correctly [also fixed it so the new GX.games preferences work]
...
- Workspaces: "Syntax Errors" command on the menubar will now open the Feather Messages window if it's actually Feather which is enabled at the time, rather than the old Syntax Errors system
...
- Building Projects: "Draw Order" value for particles is no longer ignored when actually running the game and so now particles will use the correct order [so if any of your particle systems are drawn reversed now, check if you had actually ticked that option inside the Particle Editor in the last release]
- Building Projects: "Enabled" value for particles is no longer ignored during your builds and so now disabled particles will be hidden in-game [same as above, if a particle no longer appears in-game, check you didn't previously set them to be disabled]
...
- Building Projects: GM_build_date optimisations applied to nested string template within an improperly-formatted parent template have been fixed to no longer cause a fail [#12431](https://github.com/YoYoGames/GameMaker-Bugs/issues/12431)
...
- Building Projects: [iOS] The version of Ruby used by Igor during builds is no longer out-of-date, which fixes "file not found" errors for some extensions [#9477](https://github.com/YoYoGames/GameMaker-Bugs/issues/9477)
...
- Building Projects: [YYC] `show_debug_message()` using an optional arg no longer causes a compile fail
...
- In-Game: Added a fallback for games no longer starting on some PCs as of RT 2024.14.1.253, and now the old method will be used on those affected PCs [#12921](https://github.com/YoYoGames/GameMaker-Bugs/issues/12921)
...
- In-Game: [macOS] Grave (`) key and number 2 key no longer produce the same keycode [#9686](https://github.com/YoYoGames/GameMaker-Bugs/issues/9686)
- In-Game: [macOS] Fixed instances of games silently crashing in a nested switch which return values in a "default" case [#10479](https://github.com/YoYoGames/GameMaker-Bugs/issues/10479)
...
- In-Game: `display_get_frequency()` no longer incorrectly always returns "60" on a PC when using higher refresh rate monitors
...
- In-Game: `game_set_speed()` no longer works incorrectly at FPS values >117 and when using legacy DXGI_SWAP_EFFECT_DISCARD [#13030](https://github.com/YoYoGames/GameMaker-Bugs/issues/13030)
...
- In-Game: `vertex_create_buffer_from_buffer()` no longer causes batch breaks when an empty buffer is copied to empty buffer (unless that vertex buffer was manually frozen first)
