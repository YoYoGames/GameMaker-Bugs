...
layout: home
---
# 2024.600 - Current Version: IDE 560/Runtime 577

Download Links: [Windows](https://gms.yoyogames.com/GameMaker-Installer-2024.600.0.560.exe) - [macOS](https://gms.yoyogames.com/GameMaker-2024.600.0.560.pkg) -  [Ubuntu Beta](https://gms.yoyogames.com/GameMaker-Beta-2024.600.0.560.deb)
...
- **IDE Featured Changes**
    ...
    - {% include note.html content="We are already aware that some Mac IDE users who have been using Betas for a long time may find that their settings and recent projects list, etc. have 'been lost', as you will now be using the newer location macOS has used for a few releases if you installed GameMaker from fresh - your files are all still on disk and can be moved to the new location" %}
    - Room Editor: Added an Inspector for a Room's "Layer Instance" panel
    - Text in Rooms feature [#3158](https://github.com/YoYoGames/GameMaker-Bugs/issues/3158)
    - Asset Browser/Inspector: Assets now only open in the Inspector after the click is released, rather than when pressed #5490
    ...
    - Sound Editor: [Mac IDE] GM no longer asks for Rosetta to be installed/no longer crashes on M-series Macs when creating/opening sounds if Rosetta is not installed #2592
    - Startup: "Processing User" stage takes longer than expected, so starting additional IDE instances is also slow #5671

- **Runtime Featured Changes**
    - Building Projects: [macOS] "post_run_step" script is not called at the correct time if using the Windows IDE #5453 [#5453](https://github.com/YoYoGames/GameMaker-Bugs/issues/5453)
    ...
    - Switch to the docking branch of imgui #4905
    - In-Game: Ellipse collision type does not register collisions inside the bbox but outside the sprite bounds [see comments, as this is a bigger change than it sounds] #5468
