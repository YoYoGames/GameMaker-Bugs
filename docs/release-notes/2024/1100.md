...
- **macOS Monterey Support Attempt**
    - Just the one change in this release, which was a build pipeline attempt for Monterey to remain supported for 2024.11 [#8669](https://github.com/YoYoGames/GameMaker-Bugs/issues/8669)
    - {% include important.html content="However, this was later found to be insufficient, as while the IDE will start, you cannot build macOS projects (VM and YYC give different errors, but both fail). 2024.11+ will not support Monterey or older macOS releases (we have seen that our Monterey Macs occasionally allow GameMaker to start once after a fresh installation, but then says thereafter than you need to use a newer version of macOS to if you want to start GameMaker - this dialog is what you _should_ see every time now)." %}
