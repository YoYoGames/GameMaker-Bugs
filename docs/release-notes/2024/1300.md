...
layout: home
---
# 2024.1300 - Current Version: IDE 775/Runtime 781

Download Links: [Windows](https://gms.yoyogames.com/GameMaker-Installer-2024.1300.0.775.exe) - [macOS](https://gms.yoyogames.com/GameMaker-2024.1300.0.775.pkg) - [Ubuntu Beta](https://gms.yoyogames.com/GameMaker-Beta-2024.1300.0.775.deb)
...
- **UI Layers Changes/Fixes**
    - UI Layers: Allow layer_x and layer_y to work with UI layers #9668
    - UI Layers: Create events for instances on layers that target viewports are not fired in 2024.1300 Beta 5 #9711
    - UI Layers: Runtime layout does not apply Room Editor scaling values correctly when positioning instances #9737
    - UI Layers: Mouse Enter event performed on game start for some instances on UI layers #9666
    - UI Layers: Tiling a sprite asset in a flexpanel causes compile to fail #9725
    - UI Layers: Still laid out every frame even when non-visible #9580
    - UI Layers: "depth" affects draw order on a layer, add layer_get_type() and is_on_gui_layer() so this can be determined and avoided #9626
...
- **Code Editor 2 and Feather Fixes**
    ...
    - Feather: Match3 template gives a Language Services Error each time you create a project from it in Beta IDE v2024.1300.0.768 #9653 / Code Editor 2: LSP failing in projects it did not fail in in previous beta #9655
    - Code Editor 2: GM unstable when accepting an autocomplete "IDE.OnWeakDelegateException - FreeType error: Invalid reference., glyphIndex ..." #9723
    - Debugger: [Code editor 2] Stepping into multiple scripts and then taking focus off the Debugger workspace permanently breaks the menu bar #9733
    - Code Editor 2: Autocomplete shows previous incomplete/previous variants of variables no longer used #9478
    - Code Editor 2: Creating a JSDoc header displays information weirdly in 2024.1300 Beta 5 #9692

- **SVG Support Additions**
    - Building Projects: SVG sprites should not export an unwanted bitmap copy also #9573
    - SVGs: Warn on import if the incoming image contains unsupported features #9572
    - Sprite Editor: SVGs containing pure black colours import this as a silver/grey, passes incorrect result in-game also #8126

- **Other IDE Changes To Be Aware Of**
    - Project Load: Importing some .yyzs made with 2024.1300 Betas can cause 2024.11 to lock up (note that the fix here is to prevent the situation happening in 2024.13 and future versions - 2024.11 will not be fixed) #9321
    - Project Save: Deleting an Object used as a "following" in a viewport corrupts the project, making it give compile and load errors #8652
    - Redefine Keys: [Mac IDE] Mapping "Start/End of Document" to Command-Up/Down keys does not work #9112
    - Room Editor: UI Layers panel Paste hotkey uses same combination as Copy, so cannot be used #9629
    - Tile Set Editor: GM crash when using an external editor to enlarge a sprite used by a tileset #9052
    - UI/UX: [Mac IDE] Make Option+Right Arrow position caret at end of current word, not start of next word #9099

- **Other Runtime Changes To Be Aware Of**
    - Building Projects: Attached particles-heavy project fails with "Object reference not set to an instance of an object" in 2024.1300 runtimes only #9657
    - Building Projects: [Android YYC, IOS YYC] variable_struct_set() causes fails with "no matching function for call" error #9596
    - In-Game: Crash when accessing/modifying a function/constructor static variable from objects in 2024.13 #9659
    - In-Game: [VM] Bitwise "or" operator not applying correctly to some variables when using 2024.1300 Beta 5 #9671
    - In-Game: asset_..._tags() variants neither error nor add a tag if an asset ref is passed (note that these functions all now also throw a code error in the case of invalid arguments, rather than silently ignoring) #9375
    - In-Game: os_request_permission()'s DeviceMotion "response" values are too similarly-worded and should be clarified #9635
