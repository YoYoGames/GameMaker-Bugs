...
- **Add Sprites via URL**
	- You can now use URL in sprite_add and sprite_replace to add and replace your sprites in game 
	- For more info see [#9344](https://github.com/YoYoGames/GameMaker-Bugs/issues/9344 )

- **Ability to call GML constants from JS**
	- GML built-in variables have been added to the gml object returns from __js_get_gml() this allows you to now do:

    `var gml = __js_get_gml()`
    `console.log( gml.delta_time );`
    `console.log( gml.font_texture_page_size );`
    `gml.font_texture_page_size = 2048;`
    `console.log( gml.font_texture_page_size );`

	- {% include important.html content= “Please note that differentiate between built-ins that are global and ones that are only for instances (i.e. x, y, image_index etc ), so keep that in mind when using this functionality “ %}
