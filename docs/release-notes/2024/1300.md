...
{% include note.html content="We are aware that some projects can give a Feather LSP error notification on load in this version - e.g., the Match3 template on the Start Page #9653" %}
...
- **SVG Support Additions**
    - Added support for almost all forms of radial gradients in shapes (see the issue for one caveat) #8276
    ...
    - SVGs: Dialog for importing as a bitmap instead is not shown when dropping a .svg onto GameMaker #9584
    - Tile Set Editor: Assigning a vector sprite silently converts it to a bitmap, should show a dialog saying this will happen #9583
...
- **Game Options Default Value Changes**
    ...
    - Also, be aware that the default value of Windows "Interpolate colours between pixels" is now ON, so it matches what the manual has long said and is the same as all the other targets #2343
...
- **Changes To Noone Being Returned By Sume Functions**
    ...
    - In-Game: layer_instance_get_instance() returns "-4" when the element is not an instance, should be an invalid instance handle ("ref instance -4") #9374

- **Bug Reporter UI Tweaks**
    - Made the private attachment section an actual boxed/grouped section so we can avoid misinterpretations #8398
    - Now sends the GMRT package version when submitting a GMRT Bug report, rather than sending the GMS2 runtime version (it's not marked as closed yet, as a new GMRT release is required before this will work, but the IDE change is in this release) #8583
    - {% include note.html content="If you're still seeing Unauthorised errors following last week's Web server changes, please sign out and in again inside GameMaker and this will likely fix straight away - #9473" %}
