...
CE2 (Beta) Now Available For Use
...
- **Game Options: New controls for toggling some recent 2024.11 in-game changes back to their legacy behaviour**
    - Added new options for toggling legacy behaviours for string-to-number conversion, treatment of "other" (see below), and strict audio errors
    - See this issue for details - [#8138](https://github.com/YoYoGames/GameMaker-Bugs/issues/8138)

- **Changes To Some Funtions Which Accept Numbers-As-Strings**
    - In-Game: json_parse() now throws an error if a string starting with a digit is passed [#8015](https://github.com/YoYoGames/GameMaker-Bugs/issues/8015)
    - In-Game: real() now throws errors for strings that start with a number and have trailing characters (e.g., "infobar", "nano", "42 apples") [#7685](https://github.com/YoYoGames/GameMaker-Bugs/issues/7685)

- **Static initialisation no longer runs multiple times as of 2024.11, so added a method of restoring old behaviour**
    - Added a new pragma command for enabling this old behaviour - `gml_pragma("AllowReentrantStatic", true);` 
...
- **Other Runtime Changes To Be Aware Of**
    ...
    - In-Game: get_save_filename() freezes the game when "Cancel" is clicked / does not return an empty string [#7988](https://github.com/YoYoGames/GameMaker-Bugs/issues/7988)
