...
## IDE Other Featured Changes

    - Install: Nov 23 EULA has been added
    - Install: tvOS runtime installation can cause silent crashes, possibly due to length of file path [#2077](https://github.com/YoYoGames/GameMaker-Bugs/issues/2077)
    ...
    - Stability: GameMaker has an intermittent fatal crash with audio threads in 2022.0.2 IDEs [#2394](https://github.com/YoYoGames/GameMaker-Bugs/issues/2394)
    - Start Page: "Last Login" text in LTS IDEs always says "Currently Offline" even when freshly logged-in [#7275](https://github.com/YoYoGames/GameMaker-Bugs/issues/7275)

## Runtime Other Featured Changes

    - Building Projects: [YYC] Compile fails if using an array item as the default value for a function argument [#7171](https://github.com/YoYoGames/GameMaker-Bugs/issues/)
    - Building Projects: [iOS] Xcode projects should target C++ version 17
    - Building Projects: [Android] Google Play Asset Delivery does not export dynamic texture pages correctly, gives black overlay in-game [#2171](https://github.com/YoYoGames/GameMaker-Bugs/issues/2171)
    - In-Game: Assigning a variable a built-in value (x, y, health, score etc) in the Create event causes errors when attempting to use the variable later
    - In-Game: Crash when accessing named instances in rooms created before GM v2.3.0
    ...
    - In-Game: [HTML5] HTTP Async event is broken in LTSr1 and r2 [#4925](https://github.com/YoYoGames/GameMaker-Bugs/issues/4925)
    - In-Game: [iOS] Mouse\touch returned values are incorrect
    - In-Game: [macOS] audio_create_stream() sounds crash the game in 2022.0.2 LTS [#5876](https://github.com/YoYoGames/GameMaker-Bugs/issues/5876)
    - In-Game: audio_get_recorder_count() does not count any devices which contain non-Latin characters in their name [#5124](https://github.com/YoYoGames/GameMaker-Bugs/issues/5124)
    - In-Game: collision_rectangle() does not work correctly with precise collisions in 2022.0.2 LTS [#6179](https://github.com/YoYoGames/GameMaker-Bugs/issues/6179)
    - In-Game: network_send_broadcast() packets do not come from the port expected for the specified socket [#2480](https://github.com/YoYoGames/GameMaker-Bugs/issues/2480)
    - In-Game: [HTML5] sprite_delete() causes crash if using LTS runtimes [#7177](https://github.com/YoYoGames/GameMaker-Bugs/issues/7177)
