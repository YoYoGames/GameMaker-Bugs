...
_And remember, if a fix/feature you need is not listed in these notes, then please continue using the regular monthly release, as LTS releases deliberately do not contain everything found in newer GameMaker versions._
...
- For all of the below, you will need to update your SDK installation to this version and potentially edit your Preferences paths to match, otherwise builds _will_ fail
...
- However, note that these changes are consistent with 2023.8.1’s contents and so we are aware some specific models of phones continue to override 120hz requests and revert the screen back down to 60 after a short delay
